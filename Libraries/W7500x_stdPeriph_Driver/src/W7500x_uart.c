...
void UART_StructInit(UART_InitTypeDef* UART_InitStruct, uint32_t buad)
{
  /* UART_InitStruct members default value */
  UART_InitStruct->UART_BaudRate = buad;
...
uint32_t UART_Init(UART_TypeDef *UARTx, UART_InitTypeDef* UART_InitStruct)
{
	...
	CRG->UARTCLK_SSR = CRG_UARTCLK_SSR_MCLK;	// Set UART Clock using PLL output clock (MCLK)
	
	
	m = (CRG->PLL_FCR>>16)&0xff;
	n = (CRG->PLL_FCR>>8)&0xff;
	
    uartclock =  (8000000UL) * m/n     /  (1 << CRG->UARTCLK_PVSR);
